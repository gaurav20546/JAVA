...
public class NumberTORoman 
{
	...
	public static void main(String[] args)
	{
		...
		NumberTORoman s= new NumberTORoman();
		System.out.println("Enter the number to convert into roman");
		int n=sc.nextInt();
	...
	// String result="";
	 ...
	 public String intToRoman(int num) 
	 {
		 ...
		 String[] thousands = {"", "M", "MM", "MMM"};
	        ...
	        // Split the number into places (thousands, hundreds, tens, ones)
	        String result = thousands[num / 1000]   // Handle thousands place
	                        + hundreds[(num % 1000) / 100]  // Handle hundreds place
	                        + tens[(num % 100) / 10]  // Handle tens place
	                        + ones[num % 10];  // Handle ones place
...
//		 {
//			 String s="V";
//			// int firstDigit = Character.getNumericValue(length);
...
If the value starts with 4 or 9 use the subtractive form representing one symbol subtracted from
 the following symbol, for example, 4 is 1 (I) less than 5 (V): IV and 9 is 1 (I) less than 10 (X): 
 IX. Only the following subtractive forms are used: 4 (IV), 9 (IX), 40 (XL), 90 (XC), 400 (CD) and 
 900 (CM).
